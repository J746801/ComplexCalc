...
3. For Linux: Download the latest ComplexCalc.bin file, and if this is the first time downloading it, download the .desktop (the shortcut) and .svg (the icon) files and change the file paths in the .desktop file. Please note the desktop file should be in /usr/share/applications/ or /usr/local/share/applications/ for system-wide or ~/.local/share/applications/ for user-specific installation, and the icon (.svg) should be in /usr/share/icons/<theme-name>/scalable/apps/ for system-wide or ~/.local/share/icons/<theme-name>/scalable/apps/ for user-specific installation.
